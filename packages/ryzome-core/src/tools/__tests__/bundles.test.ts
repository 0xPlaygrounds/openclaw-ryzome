...
describe("bundle tools", () => {
	...
	it("creates a bundle with its initial document order", async () => {
		const createSpy = vi
			.spyOn(RyzomeClient.prototype, "createDocument")
			.mockResolvedValue(bundle as never);
		const result = await executeCreateBundle(
			{ title: "Research", document_ids: ["doc123"] },
			clientConfig,
		);
		expect(createSpy).toHaveBeenCalledWith(
			expect.objectContaining({
				content: { _type: "Bundle", _content: { ids: ["doc123"] } },
			}),
		);
	...
	it("gets and lists bundles", async () => {
		...
		const getResult = await executeGetBundle(
			{ bundle_id: "bundle123" },
			clientConfig,
		);
	...
	it("patches bundle membership before returning the refreshed bundle", async () => {
		const patchSpy = vi
			.spyOn(RyzomeClient.prototype, "patchBundle")
			.mockResolvedValue(undefined);
		vi.spyOn(RyzomeClient.prototype, "getDocument").mockResolvedValue(
			bundle as never,
		);
		await executeUpdateBundle(
			{
				bundle_id: "bundle123",
				add_document_ids: ["doc456"],
				remove_document_ids: ["doc789"],
				document_ids: ["doc456", "doc123"],
			},
			clientConfig,
		);
