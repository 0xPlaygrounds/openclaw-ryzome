...
export const updateBundleToolDescription =
	"Add, remove, or reorder documents in a Ryzome bundle. document_ids replaces only the order and must include exactly the bundle's current document IDs.";

export const updateBundleParamsSchema = z.object({
	bundle_id: z.string().describe("The ID of the bundle to update"),
	add_document_ids: z.array(z.string()).optional().describe("Document IDs to add"),
	remove_document_ids: z.array(z.string()).optional().describe("Document IDs to remove"),
	document_ids: z.array(z.string()).optional().describe("Full desired document order"),
});

export async function executeUpdateBundle(rawParams: unknown, clientConfig: RyzomeClientConfig) {
	const params = updateBundleParamsSchema.parse(rawParams);
	const operations = [
		...(params.add_document_ids ?? []).map((id) => ({ _type: "addDocument" as const, id })),
		...(params.remove_document_ids ?? []).map((id) => ({ _type: "removeDocument" as const, id })),
		...(params.document_ids ? [{ _type: "reorderDocuments" as const, ids: params.document_ids }] : []),
	];
