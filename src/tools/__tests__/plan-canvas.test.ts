...
describe("executePlanCanvas", () => {
  ...
  it("should auto-chain steps when no ids or dependsOn are specified", async () => {
    ...
    expect(ops.filter((o: { _type: string }) => o._type === "createNode").length).toBe(3);
    expect(ops.filter((o: { _type: string }) => o._type === "createEdge").length).toBe(2);
  ...
  it("should handle a single step with no dependencies", async () => {
    ...
    const ops = patchSpy.mock.calls[0][1].operations;
    expect(ops.filter((o: { _type: string }) => o._type === "createNode").length).toBe(1);
    expect(ops.filter((o: { _type: string }) => o._type === "createEdge").length).toBe(0);
  ...
  it("should default step ids to step-{index}", async () => {
    ...
    const ops = patchSpy.mock.calls[0][1].operations;
    expect(ops.filter((o: { _type: string }) => o._type === "createEdge").length).toBe(1);
  });
});
