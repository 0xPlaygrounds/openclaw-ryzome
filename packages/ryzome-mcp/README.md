...
| `update_ryzome_bundle` | Add, remove, or reorder documents within a bundle |
...
| `ryzome://bundles` | Static | JSON list of library-visible bundle summaries (ID, title, URL) |
| `ryzome://bundle/{id}` | Dynamic | Single bundle rendered as structured markdown with its ordered documents |

The dynamic resources support `list` — MCP clients can enumerate all available canvases, documents, and bundles.
